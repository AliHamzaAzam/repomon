...
| `agent.watch_bytes` | `{ lane_id, window?, on }` | `null` — stream one pane's raw PTY bytes (tmux `pipe-pane`) as `event.agent.bytes`. Single-watch: a new `on` replaces the previous watch. The embedded renderer's feed; the capture-based `viewport.set` streaming is unaffected. |
