...
function getSystemInstallCommand(tool: "tmux" | "git"): string {
  if (isMac()) {
...
function getAgentInstallInfo(kind: string, command: string): { command: string; guide?: string } | null {
...
export default function SettingsModal(props: SettingsModalProps) {
  ...
  // Auto-fetch doctor on tab switch or mount if initialTab === "system"
  createEffect(() => {
    if (tab() === "system") {
      void fetchDoctor();
    }
                      ...
                      <div class="space-y-5">
                            ...
                            <div class="p-3.5 space-y-2">
                              <div class="flex items-start justify-between gap-3">
                                <div class="flex items-center gap-2.5 min-w-0">
                                  <div class="flex size-7 items-center justify-center rounded-lg border border-line bg-surface text-foreground shrink-0">
                                  ...
                                  <div class="min-w-0">
                                    ...
                                    <p class="font-mono text-[11px] text-muted truncate mt-0.5" title={tmuxInfo().path || undefined}>
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                  <Show when={tmuxInfo().version}>
                                    {(ver) => (
                                      <span class="font-mono text-[11px] text-muted bg-surface px-2 py-0.5 rounded border border-line">
                                        {ver()}
                                      </span>
                                    )}
                                  </Show>
                                  <Show
                                    ...
                                    fallback={
                                      <span class="rounded bg-amber-500/10 border border-amber-500/30 px-2 py-0.5 text-[11px] font-medium text-amber-500">
                                        Missing
                                      </span>
                                    }
                                  >
                                    <Show
                                      ...
                                      fallback={
                                        <span class="flex items-center gap-1.5 rounded bg-emerald-500/10 border border-emerald-500/30 px-2 py-0.5 text-[11px] font-medium text-emerald-500">
                                    ...
                                    >
                                      <span
                                        class="flex items-center gap-1.5 rounded bg-accent/15 border border-accent/30 px-2 py-0.5 text-[11px] font-medium text-accent font-mono"
                              ...
                              <Show when={tmuxInfo().available && tmuxInfo().source === "bundled"}>
                                <div class="flex items-center gap-1.5 text-[11px] text-accent/90 bg-accent/8 rounded-md px-2.5 py-1 border border-accent/20">
                              ...
                              <Show when={!tmuxInfo().available}>
                                <div class="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-xs space-y-2">
                                  <p class="text-foreground">
                                    Repomon needs tmux to run agent sessions and live terminal attachments.
                                  </p>
                                  <div class="flex items-center gap-2">
                                    <code class="flex-1 font-mono text-[11px] bg-surface px-2.5 py-1 rounded border border-line select-all text-foreground">
                                      {getSystemInstallCommand("tmux")}
                                    </code>
                                    <button
                                      type="button"
                                      class="focus-ring flex items-center gap-1 rounded-md border border-line bg-surface px-2.5 py-1 text-xs font-medium text-foreground hover:bg-line/40 transition-colors"
                                      onClick={() => void copyToClipboard("tmux-install", getSystemInstallCommand("tmux"))}
                                      aria-label="Copy tmux install command"
                                    >
                                      <Show when={copiedKey() === "tmux-install"} fallback={<IconCopy size={12} class="text-muted" />}>
                                        <IconCheck size={12} class="text-emerald-500" />
                                      </Show>
                              ...
                              <div class="flex items-start justify-between gap-3">
                                <div class="flex items-center gap-2.5 min-w-0">
                                  ...
                                  <div class="min-w-0">
                                    ...
                                    <p class="font-mono text-[11px] text-muted truncate mt-0.5" title={gitInfo().path || undefined}>
                              ...
                              <Show when={!gitInfo().available}>
                                <div class="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 text-xs space-y-2">
                                  ...
                                  <div class="flex items-center gap-2">
                                    ...
                                    >
                                      <Show when={copiedKey() === "git-install"} fallback={<IconCopy size={12} class="text-muted" />}>
                                        <IconCheck size={12} class="text-emerald-500" />
                                      </Show>
                        ...
                        <div class="rounded-xl border border-line bg-surface p-4 space-y-4">
                          <div class="flex items-center justify-between">
                            ...
                            <span class="text-[11px] font-mono text-muted bg-surface-raised px-2 py-0.5 rounded border border-line">
                          ...
                          <div class="grid gap-2.5 sm:grid-cols-2">
                            <For each={agentsList()}>
                              {(agent) => {
                                ...
                                return (
                                  <div class="flex flex-col justify-between rounded-lg border border-line/70 bg-background/50 p-3 space-y-2">
                                    ...
                                    <Show when={!agent.detected}>
                                      <div class="border-t border-line/40 pt-2 text-[11px] text-muted space-y-1.5">
                                        <Show
                                          when={installInfo()}
                                          fallback={
                                            <p class="text-muted/80">
                                              Install binary or configure custom command in <button type="button" class="underline hover:text-foreground" onClick={() => setTab("agents")}>Agents & Icons</button>.
                                            </p>
                                          }
                                        >
                                          {(info) => (
                                            <div class="space-y-1">
                                              <p class="text-muted/90 truncate">{info().guide}</p>
                                              <div class="flex items-center gap-1.5">
                                                <code class="flex-1 font-mono text-[10px] bg-surface px-1.5 py-0.5 rounded border border-line select-all text-foreground truncate">
                                                  {info().command}
                                                </code>
                                                <button
                                                  type="button"
                                                  class="focus-ring flex items-center gap-1 rounded border border-line bg-surface px-1.5 py-0.5 text-[10px] font-medium text-foreground hover:bg-line/40 transition-colors shrink-0"
                                                  onClick={() => void copyToClipboard(agentKey, info().command)}
                                                  aria-label={`Copy install command for ${agent.name}`}
                                                >
                                                  <Show when={copiedKey() === agentKey} fallback={<IconCopy size={11} class="text-muted" />}>
