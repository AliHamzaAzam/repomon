...
At a Conversation pane width of (1200px), a context rail (320px) shows environment, working changes, files, agents, and repository pull requests. Below that threshold, compact repository/branch context sits above the composer. At pane widths up to (800px), ledger padding becomes (20px), the gutter becomes (52px), and the gap becomes (12px); the composer model/send group is capped at (45%) instead of (55%). These are container queries so split panes and an open editor receive the same compact behavior.
