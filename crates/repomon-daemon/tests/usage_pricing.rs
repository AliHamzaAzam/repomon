...
async fn cached_snapshot_and_undated_overrides_reprice_historical_summary() {
    ...
    unsafe {
        std::env::set_var("REPOMON_DATA_DIR", dir.path());
    }
    let cache = dir.path().join("prices");
    std::fs::create_dir_all(&cache).unwrap();
    std::fs::write(
        cache.join("litellm.json"),
        include_str!("fixtures/model_rates_snapshot.json"),
    )
    .unwrap();
    ...
    let before = dispatch(&ctx, &sess, "usage.summary", Some(params.clone()))
        .await
        .unwrap();
    assert_eq!(
        before["totals"]["cost_usd"], 36.0,
        "snapshot must price stored events before overrides"
    );
    for model in models {
        dispatch(
            &ctx,
            &sess,
            "config.set",
            Some(json!({ "usage_price_override_upsert": {
    ...
    let after = dispatch(&ctx, &sess, "usage.summary", Some(params))
        .await
        .unwrap();
    assert_eq!(after["totals"]["cost_usd"], 22.0);
    let groups = after["groups"].as_array().unwrap();
    assert_eq!(
        groups.iter().find(|r| r["key"] == models[0]).unwrap()["totals"]["cost_usd"],
        12.0
    );
    assert_eq!(
        groups.iter().find(|r| r["key"] == models[1]).unwrap()["totals"]["cost_usd"],
        10.0
    );
